...
- The first session **Losses and Metrics** will cover how to *select* a metric. 
- The second session **Testing** will cover how to *estimate* a metric.
...
^1^This is a generalisation statement (based on testing data).


- **Question**: Is 80% or 20% accuracy good or bad?
- **Answer**: Depends on the **goal**!

## Goals

Consider an example where the model is predicts **tumour** (positive prediction) or **no tumour** (negative prediction) from a medical image. There are two types of error:
...
- Correct Positive Prediction (TP)
- Correct Negative Prediction (TN)
- Incorrect Positive Prediction (FP)
- Incorrect Negative prediction (FN)
...
Consider the previous example again where the accuracy of the model is 99%. What if:

1. 99 out of 100 people in the test dataset have no tumour.
...
This leads to estimating metrics with testing sets
...
- **Answer**: When the estimated metric does not accurately reflect the true metric.
...
The estimated metric does not accurately reflect the true metric when the sample used to estimate the metric is no appropriate. The sample (i.e. test dataset) should be

1. Independent of the training dataset
2. Representative of the population

This is usually done by taking from the available data a  random $(1-p)$% for the training dataset and $p$% for the testing dataset. For example $p=80$ is common.

## Independent

- **Question**: If there are no common data points in the training and testing datasets are they independent?
- **Answer**: Not necessarily. Independence requires that information in the training dataset is not contain in the testing dataset. 
...
This can *only* be assessed by reviewing the code. Typically you would have to trust that the Machine Learning Engineers avoided this issue. However, these issues are often subtle and are not avoided by even the most experience.

## Representative

- **Question**: If the testing dataset is a random subset of the available data is the testing dataset representative?
- **Answer**: Not necessarily. In classification when positive class is rare (e.g. tumour) sometimes the testing dataset might not include any positive cases!
...
Again, this can only be assessed by review the code. Typically you would have to trust that the Machine Learning Engineers avoided this issue. 
...
- **Answer**: No necessary. Issues can arises at deployment and during deployment. 
